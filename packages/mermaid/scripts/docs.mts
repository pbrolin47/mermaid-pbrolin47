...
// Use createRequire so the path is resolved relative to this file, not process.cwd().
// This makes the test suite (which runs from the repo root) work regardless of the
// repository's directory name.
const requireFromHere = createRequire(import.meta.url);
export const MERMAID_RELEASE_VERSION = (requireFromHere('../package.json') as { version: string })
  .version;
